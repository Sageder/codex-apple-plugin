...
export function scoreSummary(message, query) {
...
export function chunkText(content, maxChars = 1200) {
...
export function rankContext(messages, query, topK) {
    const terms = tokenize(query);
    const snippets = [];
...
function buildReason(input) {
    ...
    if (input.baseScore > 0)
        reasons.push("metadata matched");
    if (input.bodyScore > 0)
        reasons.push(`body matched ${input.matchedTerms.slice(0, 6).join(", ")}`);
